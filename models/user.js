...
class User {
  ...
  static async register({username, password, first_name, last_name, phone}) {
    const encrypted_pass = await bcrypt.hash(password, BCRYPT_WORK_FACTOR)
    ...
    const result = await db.query(
      ...
      [username, encrypted_pass, first_name, last_name, phone])
  ...
  static async authenticate(username, password) {
    ...
    if (!result.rows[0]) {
      ...
      const hashed_pass = result.rows[0].password
